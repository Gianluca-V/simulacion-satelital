...
function Content({ msg }: { msg: any }) {
  ...
  return (<>{msg.failureReason && <div style={styles.failReason}>⛔ {msg.failureReason}</div>}<div style={styles.status}>Estado: <span style={{ color: ok && !msg.failureReason ? '#0f8' : '#f44' }}>{ok && !msg.failureReason ? '✅ Completado' : '❌ Fallido'}</span></div>
